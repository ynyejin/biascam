...
BiasCam은 생성된 직캠 영상을 웹에서 재생 가능한 형식으로 변환하기 위해 FFmpeg를 사용합니다.  
FFmpeg는 Python 패키지가 아니므로 운영체제에 맞게 별도로 설치해야 합니다.
...
* 테스트용 샘플 영상은 `backend/data/samples/` 폴더에 포함되어 있으며, 별도의 영상이 없어도 샘플을 통해 바로 사용할 수 있습니다.
* 직접 테스트할 경우 여러 명이 함께 등장하는 짧은 단체 퍼포먼스 영상을 업로드하면 됩니다. MOV도 가능하지만, 안정적인 실행을 위해 MP4 형식을 권장합니다.
